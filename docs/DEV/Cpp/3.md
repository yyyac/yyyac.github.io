...
此例中，将reset()声明为Accumulator类的友元，所以它可以访问类的私有成员。但必须向reset()传递一个Accumulator对象，因为它不是成员函数，没有*this指针，除非给定一个。
...
但此时`displayItem`使用`Storage`作为引用参数。当把Storage的定义移到Display后面时，编译器不知道Storage是什么，此时有两种方案：
